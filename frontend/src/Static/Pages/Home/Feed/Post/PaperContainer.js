...
class PaperContainer extends Component {
  render() {
    return (
      <Post>
        <Link to={"/papers/" + this.props.data.slug}>
          <Title>{this.props.data.name}</Title>
          <Description>{this.props.data.abstract.substring(0, 100)}...</Description>
          <Meta>Paper from {moment(this.props.data.created_at).fromNow()}</Meta>
        </Link>
      </Post>
    )
  }
